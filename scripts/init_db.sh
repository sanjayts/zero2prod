...
if ! [ -x "$(command -v psql)" ]; then
  # Use brew install libpq on Mac M1s to install psql without the postgres server
...
if ! [ -x "$(command -v sqlx)" ]; then
  ...
  echo >&2 "Use:"
  echo >&2 " cargo install sqlx-cli --no-default-features --features postgres,native-tls"
